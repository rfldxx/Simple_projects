...
<summary>305 символов (15.04.2025)</summary>
...
<summary>290 символов (ОМГ, год спустя: 19.05.2026)</summary>
...
Ахахаха, я оказывается неправильно передставлял себе `set.end()` - в итоге можно снести последний элемент `{i, 1, j}` в инициализации set-а (спасибо deepseek).
```cpp
#include<bits/stdc++.h>
using namespace std;
...
Дано корневое дерево. Надо оффлайн отвечать на запросы: $(u, k)$ - количество вершин поддерева $u$, отдаленных от $u$ на растоянии не более чем $k$ рёбер ($k$ для каждого запроса своё).
...
1. Если для числа (с возможным изменением) нам дополнительно известен (богом дан) 1 контрольный бит, то можно детектировать изменение порядка байт на противоположный (см. функцию reap):
...
<summary> todo: МОЙ ЛЮБИМЫЙ монотонный стек  (+ ещё стеки) </summary>
...
**Еще задачки на стеки:** (типо в них можно моделировать очереди через два стека без ухудшения асимптотики, хотя по хорошему бы уже прочитать про [очередь на 6 стеках](https://neerc.ifmo.ru/wiki/index.php?title=Персистентная_очередь#.D0.A0.D0.B5.D0.B0.D0.BB.D0.B8.D0.B7.D0.B0.D1.86.D0.B8.D1.8F_.D0.BE.D1.87.D0.B5.D1.80.D0.B5.D0.B4.D0.B8_.D0.BD.D0.B0_.D1.88.D0.B5.D1.81.D1.82.D0.B8_.D1.81.D1.82.D0.B5.D0.BA.D0.B0.D1.85))


([задача](https://www.codechef.com/COOK82/problems/COOK82C)) Дан массив положительных чисел. Его последовательно изменяют пока в нём есть ненулевые элементы. \
На каждом шаге в нём находится максимальный элемент и делится нацело на $2$. \
Даны запросы - какое число было выбрано на шаге $t$?

<details>

<summary> Решение </summary>

Всего шагов будет $O( n \cdot \log_2 A)$, так что если мы сможем *быстро* находить максимум то можем просто промоделировать все шаги.

Оказывается мы можем извлекать максимум в нашем процессе за $O(1)$ (+ первоначальная сортировка исходного массива). \
Заведём две отсортированные очереди: в первой храняться числа из промежутка $[mx, mx/2)$, во второй все отсальные числа (т.е. числа $\le mx/2$). \
Оказывается мы можем поддерживать этот инвариант для каждого шага за $O(1)$ амортизированно.
...
ll mx = 0;
vector<ll> a(n);
for(auto& e : a) { cin >> e; mx = max(mx, e); }

deque<ll> p , q;    // в p - элементы большие чем mx/2
for(auto  e : a) (e > mx/2 ? p : q).push_back(e);
for(auto  e : {&p, &q}) sort(e->begin(), e->end(), greater());

q.push_back( -1 );  // чтобы не делать проверку !q.empty()

int curr = 1, need;
while( Q-- ) {
    for( cin >> need; curr < need; curr++ ) {
        ll k = p.front(); p.pop_front();

        (p.empty() ? p : q).push_front( k/2 );

        while( q.front() > p.front()/2 ) {
            p.push_back( q.front() );
            q.pop_front();
        }
    }
...
Только при оформлении понял, что такое решение очень похоже на классическую задачу о гоблинах и шаманах (только там "половина" берется не по значению, а по количеству).

Ещё такой же идеей можно постоить [код Хаффмана за O(n)](https://neerc.ifmo.ru/wiki/index.php?title=Алгоритм_Хаффмана_за_O(n)):
1. Отсортируем исходный массив. Создадим ещё один массив который будет использоваться только для "сумм".
2. На каждом шаге мы хотим извлеч два минимума и добавить их сумму в нашу "структуру". Есть три возможные пары минимумов:
...
3. Их сумму добавляем в конец второго массива. Весь процесс обеспечивает нам что добавляемые суммы только возрастают, поэтому второй массив остается отсортированным на каждом шаге.
...
#### Сдвинутое решето Эратосфена

Что если нам надо найти все простые числа на отрезке $[x, x + 10^6]$, где $x$ может быть порядка $10^{12}$?

<details>
<summary> Уже всё проспойлеренно </summary>

Удивительно - можно всего лишь "сдвинуть" решето!

Обозначим искомый отрезок за $[l, r]$. \
Предподсчитаем с помощью обычного решета все простые числа $p$ до $\sqrt{r}$. \
Если число $a \in [l, r]$ не делится ни на одно $p$ - то $a$ простое число. \
Как и в обычном решете будем помечать с шагом $p$ - итого сложность: $\frac{ \Delta }{2} + \frac{ \Delta }{3} + \frac{ \Delta }{5} + \dots + \frac{ \Delta }{p} + \dots = O(\Delta \cdot \log ( \log (\Delta)) + \sqrt{r} ) $. Корень из $r$ взялся, так как некоторые простые числа могут рассмотреться вхолостую/единожды (если $p > \Delta$).

**Задача:** \
На отрезке $[l, r]$ найти количество чисел у которых количество делителей является простым числом. \
Ограничения: $1 \le l \le r \le 10^{12}$ и $|r-l| \le 10^6$  \
(например: для $16$ делители это { $1, 2, 4, 8, 16$ } - их $5$ что является простым числом) 
...
// нужно для проверки на простоту кол-ва делителий (достаточно было бы ln_2(10^12)+1)
int is_prime[100];
...
int main() {
    ...
    while( t-- ) {
        ...
        int L = r-l+1;
        int ans = 0;
        int sieve[N+1] = {};  // эхх, прочитать бы уже наконец про memset
        for(auto p : primes) {
            ll start = 1LL* ( (l+p-1)/p )*p;
            for(ll x = start; x <= r; x += p) {
                if( sieve[x-l] ) continue;
                sieve[x-l] = 1;
            
                // что если это число вида: a = p^k
                ll a = x, k = 0;
                while( a%p == 0 ) { a /= p; k++; }
                ans += (a == 1) && is_prime[k+1];
            }
        }

        // простые числа на отрезке [l, r]
        for(ll x = l; x <= r; x++)
            ans += !sieve[x-l];  
...
А как мы эффективно можем находить позиции без техник выше? Видимо как всегда, рассмотрим метод разделяй и влавствуй. . . В итоге получилось подобие merge-sort ( надеюсь тоже с асимптотикой $O(N \text{ ln} N)$ - $upd:$ наверное это был риторический вопрос, но для полноты: у нас каждый уровень участвует максимум в $\text{ln} N$ мерджах):
