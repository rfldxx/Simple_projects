...
* Для общего $k$ : $T_k = \sum\limits_{i=1}^{k} \frac{1}{ \binom{n-1}{i-1} } \sum\limits_{j= i}^{n} \binom{n}{j}$ (даже не хочется эту муть проверять)

* Для $k=n$ : $T_n = 2^{n-1} \sum\limits_{i=0}^{n-1} \frac{1}{ \binom{n-1}{i} }$ (проверил для $n=4$)
