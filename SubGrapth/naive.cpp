...
bool test_benzol() { // типо бензол, но вместо -H подвещено -O-H
    ...
    //  C=C-C  -  6 штук
    auto test6 = unit<3>("CCC", {{1, 2, 2}, {2, 3}});
    TEST(benzol, test6, 6);

    //  _=_-_  - 12 штук
    auto test7 = unit<3>("___", {{1, 2, 2}, {2, 3}});
    TEST(benzol, test7, 12);

    //  _-_-_  - 24 штук
    auto test8 = unit<3>("___", {{1, 2}, {2, 3}});
    TEST(benzol, test8, 24);

    // //  _*_-_  - 24+12 штук  [пока не добавлен поиск произвольного кол-ва связей (atom1 * atom2)]
    // auto test9 = unit<3>("___", {{1, 2, -1}, {2, 3}});
    // TEST(benzol, test9, 36);
      OK(benzol)
}
